...
class CryptoRSABase():
    """
    Class is based on bytes as in- and output. Nevertheless tolerant against
    ...
    def __init__(self, path=None):
        if path is not None:
            self.path = path
        self.fullkey = None
        self.filename = 'localkey.pem'

    def _keyfile(self, name):
        return os.path.join(self.path, name + '_localkey.pem')

    def create_rsa_key(self, name):
        self.filename = self._keyfile(name)
